...
function htmlSubstr($html, $length) {
    ...
    for ($i = 0, $l = 0; $i < count($arr); $i++) {
        if ($i & 1) {
            if (mb_substr($arr[$i], 0, 2, 'UTF-8') == '</') {
                array_pop($tagStack);
            } elseif ($arr[$i][0] == '&') {
                $l++;
            } elseif (mb_substr($arr[$i], -2, null, 'UTF-8') != '/>') {
                array_push($tagStack, $arr[$i]);
            }
            $out .= $arr[$i];
        } else {
            if (($l += mb_strlen($arr[$i], 'UTF-8')) >= $length) {
                $out .= mb_substr($arr[$i], 0, $length - $l + mb_strlen($arr[$i], 'UTF-8'), 'UTF-8');
                break;
            } else {
                $out .= $arr[$i];
            }
    ...
    while (($tag = array_pop($tagStack)) !== NULL) {
        $out .= '</' . strtok(mb_substr($tag, 1, null, 'UTF-8'), " \t>") . '>';
    }
    return $out;
}

function prepare_post_from_user($str) {
    $str = strip_tags($str, '<b><strong><i><u><del><strike><em><center><h2><li><ol><ul><br><hr><center><img>');
    $str = preg_replace('/<([a-z][a-z0-9]*)[^>]*?(\/?)>/i', '<$1$2>', $str); // у всех тегов вырезаем атрибуты
    return $str;
}
...
function close_dangling_tags($html) {
    //СЃРЅР°С‡Р°Р»Р° Р±РµСЂРµРј РІСЃРµ РѕС‚РєСЂС‹С‚С‹Рµ С‚РµРіРё
    preg_match_all("#<([a-z]+)( .*)?(?!/)>#iU", $html, $result);
    $openedtags = $result[1];

    // РїРѕСЃР»Рµ РІСЃРµ Р·Р°РєСЂС‹С‚С‹Рµ
    preg_match_all("#</([a-z]+)>#iU", $html, $result);
    $closedtags = $result[1];
    $len_opened = count($openedtags);

    # РІСЃРµ С‚РµРіРё Р·Р°РєСЂС‹С‚С‹
